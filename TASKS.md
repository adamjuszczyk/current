...
- [x] **5.1** Focused on Active projects, any number at once.
  - `blocks.focused` already existed from `0002_v2.sql` — no schema change this phase. A toggle button in `BlockEditPopup` (label "Focus"/"Focused") writes it via `useUpdateBlock`; turning it on outside Active is refused with `pushError('Focused is only available on Active projects.')` rather than hidden, matching how 4.5's status-change refusal is surfaced (an attempt-and-refuse message, not a disabled control).
  - **Acceptance verified 2026-09-11** against a scratch Postgres 16 (`0001`+`0002`+`0003`, real RLS via `SET LOCAL ROLE authenticated`), driving `vite dev` in headless Chromium: toggling Focus on three separate Active projects (Alpha, Epsilon, Beta) across two Areas each persisted `focused = true` in the database, confirmed by direct query, not the DOM.
- [x] **5.2** Mutually exclusive with Waiting. Turning one on while the other is on is refused, and says which one has to be cleared first — the spec says "turning one on requires turning the other off first", so nothing is silently swapped or discarded on the user's behalf.
  - Two guards, one per direction, both in `BlockEditPopup.tsx`: `handleToggleFocused` refuses turning Focused on when `isProjectWaiting(entries)` is true; `handleOpenWaitingPopup` refuses opening the "+ Waiting entry" popup — before the user can even type — when `entries.length === 0 && block.focused` (adding a *first* entry is exactly "turning Waiting on"; once a project already has entries it was never Focused to begin with, since this same guard already refused that transition). Both refuse via `pushError` naming the other state by name, and neither swaps, clears, or discards anything on the user's behalf — the blocked action simply doesn't happen.
  - **Acceptance verified 2026-09-11**, both directions, against real database state (not just a UI message):
    - Focusing Beta (which holds a seeded Waiting entry) was refused; the banner read *"Can't turn on Focused while Waiting: turn off Waiting first."* (contains "Waiting"); `blocks.focused` for Beta stayed `false` in the database.
    - Opening "+ Waiting entry" on Epsilon (freshly Focused, zero entries) was refused before the popup ever rendered — confirmed by counting `text=New Waiting entry` in the DOM (0) — with the banner reading *"Can't turn on Waiting while Focused: turn off Focused first."* (contains "Focused"); `waiting_entries` for Epsilon stayed empty in the database.
    - **Clearing the blocker then allows the other, both directions:** unfocusing Epsilon allowed a Waiting entry to be created on it immediately after (`waiting_entries` row appeared, text `"waiting on a decision"`, confirmed by query); deleting Beta's Waiting entry through its own confirm-gated delete (4.9) then allowed Beta to be Focused (`blocks.focused` became `true`, confirmed by query).
- [x] **5.3** Marking a Focused project Done clears Focused with it.
  - `handleStatusChange` folds `focused: false` into the *same* `useUpdateBlock` call when `status === 'done' && block.focused`, rather than issuing a second mutation a failure could leave half-applied.
  - **Acceptance verified 2026-09-11**: Focused Alpha, then clicked its "done" status button; `select focused, status from blocks where id = ...` returned `status = 'done', focused = false` — asserted directly against the database, not the UI's re-render. Repeated on Gamma (used again in the 5.5 check below) with the same result.
- [x] **5.4** Focused visual: an outline around the block.
  - `BlockCard.tsx` adds `outline outline-2 outline-offset-2 outline-purple-600` when `block.focused` — a real CSS `outline`, not a `ring`/box-shadow approximation, chosen because the spec's own word is "outline" and Waiting/Focused can never co-occur (5.2) so there is no case where a block needs to carry both treatments.
  - **Acceptance verified 2026-09-11** by computed style, not appearance alone, and screenshotted: a Focused block (Delta) reads `outlineStyle: "solid", outlineWidth: "2px"`; a Waiting+Ready block built in the same scratch database (Zeta — one picked-task entry, its task completed) reads `outlineStyle: "none"` and instead shows `[aria-label="Ready"]`'s badge; Delta has zero `[aria-label="Ready"]` badges. The two remain distinguishable side by side (screenshot: `canvas-focused-vs-ready.png` — Delta blue with a purple outline; Zeta dimmed-blue Waiting card with the green Ready check).
- [x] **5.5** The Focus screen — a dedicated view listing every currently-Focused project. Clicking one switches to its Area if it is in another, scrolls its position into view on the canvas, and opens its project popup.
  - "Every" is read as across all Areas, not just the active one; grouped by Area so the jump isn't a surprise.
  - The Area canvas's Filter (Phase 7) does not apply here — it is a different view, and a filter that could hide a Focused project from the screen built to list them would defeat it. Filter doesn't exist yet; nothing here reads or depends on any filter state, so there is nothing to decouple later.
  ...
  - New `useFocusedBlocks()` in `src/lib/blocks.ts` reads every `focused = true` block across every Area via the `blocks -> areas(name)` FK embed (PostgREST-style, same embed pattern `waiting.ts` already used for `blocks(name)`), not scoped to the active Area — the query itself is what makes "every" true, not client-side filtering of an Area-scoped list. `useUpdateBlock`/`useDeleteBlock` both invalidate its query key alongside their existing per-Area one. New `src/components/FocusScreen.tsx` groups the results by `area_id` and renders one project button per group. A new `uiStore` field (`activeView: 'canvas' | 'focus'`) swaps the Workspace body between `Canvas` and `FocusScreen`; a `scrollToBlockId` one-shot field, set on click alongside `setActiveAreaId`/`openPopup`, is read by a `useEffect` in `Canvas.tsx` that waits until the target block is actually present in the now-active Area's loaded `blocks` (so it retries itself across a fresh Area's query loading rather than giving up), then calls `scrollRef.current.scrollTo(...)` and clears the target.
  - **Acceptance verified 2026-09-11**, against a scratch Postgres 16 seeded with 2 Areas (Work: Alpha/Beta/Gamma/Epsilon; Home: Delta) and driven through the real `vite dev` app in headless Chromium — every assertion below read the database directly or a computed DOM property, not just "the text appeared":
    - Focused Beta (Work), Gamma (Work, seeded at canvas position `x=3000, y=2500` specifically to start off-screen), and Delta (Home) — `select ... where focused = true` returned exactly 3 rows spanning both `area_name` values `Work` and `Home`, confirmed **before** checking the screen (so the later "populated" checks below aren't vacuous).
    - The Focus screen listed all three (`Beta`, `Gamma`, `Delta`) with both `Work` and `Home` group headings present (screenshot: `focus-screen.png`).
    - With **Work** active, clicking **Delta** (in **Home**) from the Focus screen: the Home Area tab gained the active (`bg-black`) class, and the popup that opened had its name field reading exactly `"Delta"` — a genuine cross-Area jump, not a same-Area click that happened to look like one.
    - With **Home** active, clicking **Gamma** (in **Work**, off-screen at `x=3000,y=2500`) from the Focus screen: the Work tab became active, and — the part that actually exercises the scroll mechanism rather than asserting a value that's already near zero — the canvas's own scroll container measured `scrollLeft: 1880, scrollTop: 1738` afterward, nowhere near its resting `(0, 0)`, proving the block was genuinely scrolled into view rather than merely present somewhere in an unscrolled, overflowing canvas.
    - Marking Gamma **Done** cleared `focused` in the database (`status = 'done', focused = false`) and the Focus screen no longer listed it — checked **alongside** confirming Beta and Delta were still listed, specifically to rule out a vacuous pass ("Gamma absent" is meaningless if the list is empty for an unrelated reason).
  - **Not checked:** the live Supabase project (this container had no `.env` at session start, by design — a placeholder one was written for `vite dev` only, then deleted before finishing, matching every phase since Phase 3); Phase 7's Filter, which doesn't exist yet so there is nothing to couple or decouple against; keyboard/screen-reader navigation of the Focus screen beyond the accessible names used to drive it.
...
- [x] **6.1** Connect two blocks within the same Area — never across Areas. Direction is source → target, left to right, meaning "comes before". It is informational and gates nothing; what actually blocks a project is Waiting's job.
  - Built as a click-to-connect mode ("Connect blocks" toggle in the Area canvas toolbar): the first click on a block picks it as the source, the second click on a different block attempts the connection. Both blocks are read from the already-loaded, same-Area `useBlocks(areaId)`/`useConnections(areaId)` data, so a cross-Area connection is structurally impossible — there is no candidate list that could ever contain a block from another Area.
  - **Acceptance verified 2026-09-11**: seeded 3 same-Area blocks (Cyc A/B/C), connected A→B and B→C through the UI, confirmed by direct query that both rows exist with the expected `source_id`/`target_id` direction.
- [x] **6.2** Any number of incoming and outgoing connections per block.
  - No cap anywhere in the schema or client — `connections` has no unique constraint on `source_id` or `target_id` alone, only on the `(source_id, target_id)` pair. The diamond test (6.5, below) exercises this directly: `Dia B` and `Dia D` each carry both an incoming and an outgoing edge in the same graph.
- [x] **6.3** Acyclic. A connection is refused if the target already reaches the source. Enforced in the client on the create path — Postgres cannot express reachability in a check constraint, and a recursive trigger is far more machinery than a single-account sandbox warrants. The unique constraint and the self-edge check cover the two trivial cases in the database.
  - `src/lib/layout.ts`'s `wouldCreateCycle` does a directed BFS from the candidate target over existing connections, checking whether it can reach the candidate source; `src/lib/connections.ts`'s `checkConnection` calls it (plus the self-edge and duplicate-edge checks) **before** `useCreateConnection().mutate` is ever called — a refusal never reaches the network, so a refused attempt can never write a row by construction, not by relying on catching a later Postgres error.
  - **Acceptance verified 2026-09-11**: built `Cyc A→Cyc B`, `Cyc B→Cyc C` through the UI (both confirmed by query), then attempted `Cyc C→Cyc A` through the same UI. The error banner read `Couldn't save: Can't connect: "Cyc A" already reaches "Cyc C", so this would create a cycle.` — and, checked after a 400ms wait specifically to give a wrongly-permitted write time to land, `connections` for that Area still held exactly the same 2 rows as before the attempt, not 3.
- [x] **6.4** Connections render as left-to-right lines between blocks.
  - `src/components/ConnectionLines.tsx`: one absolutely-positioned `<svg>` per Area canvas, sized to the blocks' own extent, rendered *before* the `BlockCard`s in the DOM so it sits behind them with no `z-index` needed. Each connection draws two overlapping `<line>`s from the source's right edge to the target's left edge — a thin visible line with an arrowhead marker, and a wide (`strokeWidth: 14`), invisible (`stroke: transparent`) one on top purely as a bigger click target for 6.9.
  - **Acceptance verified 2026-09-11** by screenshot (`/tmp/phase6-diamond.png`, described under 6.5) — lines with arrowheads visibly connect blocks left to right, matching the direction connections were created in.
- [x] **6.5** Auto-layout by connection graph. Every block with at least one connection is positioned by the shape of its component; a block with zero connections stays freely draggable exactly as today, with nothing to compute.
  - Algorithm: longest-path layering. A block's column is the longest path to it from any source in its component; columns run left to right at a uniform pitch (`COLUMN_PITCH = 220`); within a column, blocks are ordered by `created_at` so the result is deterministic and stable across reloads (rows use a uniform pitch too, `ROW_PITCH = 110`) — an expanded Waiting card overflows its slot rather than reflowing the graph, which is why folded is the card's default.
  - Layout writes `x`/`y` back to the same block rows (`src/lib/connections.ts`'s `useCreateConnection`, in the same mutation as the edge insert). There is no second position model and no stored component anchor — `src/lib/layout.ts`'s `computeConnectionLayout` derives everything it needs from the blocks' own current `x`/`y` and the edge list, every time a connection is added.
  - **A genuine interpretation call, documented rather than guessed at silently:** 6.5/6.7 describe how a *new* component gets its shape and how a *merge* of two components behaves, but not what happens when an edge is added *within* an already-single connected component (e.g. a shortcut or fork-closing edge) — a case with no acceptance criterion of its own. `computeConnectionLayout` resolves it conservatively: only the new edge's target and everything downstream of it shift right, and only by the minimum needed to keep the target strictly right of the source (per 6.1's left-to-right meaning); anything upstream or on an unrelated branch is untouched, and an edge that already satisfies the constraint moves nothing at all. This is a documented interpretation of a genuine spec gap, not a new *product* decision (nothing about what Connect means or shows changes) — see `src/lib/layout.ts`'s own comment on `computeConnectionLayout` for the full reasoning, including why a full global recompute was rejected (it can force blocks with no relation to the new edge to move, which is itself unspecified).
  - **Acceptance verified 2026-09-11**, the diamond case specifically named in the phase brief as the one that tells longest-path and shortest-path apart: built `Dia A→B`, `A→C`, `B→D`, `C→D` through the UI. Confirmed by query: `A.x=100` (its own seeded anchor — the tie on the first edge, A↔B, resolves to the source side per 6.7), `B.x = C.x = A.x + 220` (both one column right of A, on different rows — `B.y=100, C.y=210`, a fork not an overlap), and **`D.x = A.x + 440`** — the third column, not the second, exactly what "longest path via either branch" requires (both `A→B→D` and `A→C→D` are length 2). Screenshot `/tmp/phase6-diamond.png` shows this rendered: A on the left, B/C stacked in the middle column, D on the right, all four lines drawn with correct direction.
- [x] **6.6** Dragging any connected block moves its whole component rigidly — the same delta applied to every block reachable through any connection, all persisted together. No connected block has an independent position.
  - `src/components/BlockCard.tsx`'s existing pointer-based drag (3.2/4.x) is unchanged in mechanism; on pointer-up, it computes the dragged block's undirected component via `src/lib/layout.ts`'s `buildComponent` (every block reachable through any connection, in either direction) and, when that component has more than one block, batch-writes every member's new `x`/`y` — its own current stored position plus the identical delta the dragged block moved — through `src/lib/blocks.ts`'s new `useUpdateBlockPositions`, in one mutation. A block with zero connections is its own component of one, so this is exactly its old single-row update, unchanged in effect.
  - **Acceptance verified 2026-09-11** on the four-block chain from 6.5's chain test (`Chain W→X→Y→Z`, columns confirmed strictly increasing at exactly 220px pitch beforehand): dragged `Chain Y` by (150, −60) through real mouse events. All four blocks' recorded deltas were `{dx: 150, dy: -60}` — asserted as numerically **equal**, not merely "all four moved" — and the pitch between consecutive blocks (220px, same row) was unchanged afterward, confirming the shape survived the move intact.
- [x] **6.7** Adding a connection recomputes the resulting component's internal shape but never its position. When a new edge merges two components, the larger one by block count keeps its position and the smaller is pulled into the shape around it; on a tie, the upstream (source) side's position wins.
  - `computeConnectionLayout`'s merge branch: the winning component (larger by block count, or the source's side on a tie — which also covers the base case of two never-before-connected blocks, both components size 1) is never touched at all, in any circumstance; the losing component is rigidly translated (its own existing internal shape preserved exactly, not reshaped) so its bridge endpoint lands one column-pitch from the winning side's bridge endpoint, nudged to the next free row if that slot is already occupied by an existing block in the winning component.
  - **Acceptance verified 2026-09-11**, the exact scenario the phase brief names ("connecting a lone block to that chain leaves all four exactly where they were and places the newcomer around them"): with the 4-block chain (`Chain W→X→Y→Z`, already dragged once under 6.6) still on screen, connected `Chain Z → Chain E` (E a lone, never-before-connected block) through the UI. Confirmed by query: `W`, `X`, `Y`, `Z` are all at **exactly** the coordinates they held immediately before this connection — byte-identical, checked per block, not just "the chain still looks the same" — and `E` landed at `Z.x + 220, Z.y`, i.e. pulled into place one column right of the chain's end.
- [x] **6.8** Removing a connection repositions nothing — a layout that satisfied a graph still satisfies it with one constraint fewer. Same when deleting a block removes several connections at once, even if that splits a component into more than two pieces. Any block left with zero connections becomes freely draggable again.
  - `src/lib/connections.ts`'s `useDeleteConnection` writes nothing but the `connections` delete itself — no position write of any kind, in any branch. Deleting a block removing several connections at once needs no extra code beyond what already existed: `blocks`' own `on delete cascade` on `connections.source_id`/`target_id` (from `0002_v2.sql`) removes every edge touching the deleted block in the same statement, and nothing client-side reacts to that by repositioning anything. "Freely draggable again" isn't a flag that gets flipped — `buildComponent(block.id, edges)` (6.6, above) naturally returns a singleton for any block with zero remaining connections, which is exactly the pre-Phase-6 single-row drag path.
  - **Acceptance verified 2026-09-11**: deleted the middle connection of the (now 5-block: `W-X-Y-Z-E`) chain, `Chain X → Chain Y`, by clicking its line and confirming. All five blocks' coordinates were confirmed **byte-identical** before and after the delete — not just "unchanged in shape," literally unchanged in every stored `x`/`y`. This split the graph into `{W,X}` and `{Y,Z,E}`; dragging `W` afterward moved `X` with it (still connected, its own 2-block component) by the identical delta, while `Y`/`Z`/`E` didn't move at all. Deleting the remaining `W→X` connection too, then dragging `W` again, moved `W` alone — `X` (now its own component) stayed exactly put, confirming a true zero-connection orphan drags independently, touching nothing else.
- [x] **6.9** Removing a connection is a delete, so it goes through the confirm dialog like everything else.
  - Clicking a connection's line (via the wide invisible hit-line described under 6.4) calls `useConfirm()` with "Delete this connection?" before `useDeleteConnection().mutate` — the same `ConfirmDialogProvider` gate every other delete in the app goes through, no separate mechanism.
  - **Acceptance verified 2026-09-11** as part of 6.8's own verification above: clicking a connection's line opened the confirm dialog (`Confirm`/`Cancel` buttons, `Popup`-based, same as every other delete); the row was only removed after clicking `Confirm`.
  - **Acceptance (whole-phase, verified 2026-09-11):** a four-block chain (`Chain W→X→Y→Z`) laid out left to right at exactly `COLUMN_PITCH` apart on the same row; dragging any one of them (`Y`) moved all four by the identical `{150, -60}` delta and left the pitch between consecutive blocks unchanged; connecting a lone block (`Chain E`) to that chain left all four exactly where they were (byte-identical coordinates) and placed the newcomer one column right of the chain's end; deleting the middle connection moved no block at all (byte-identical before/after), and the blocks orphaned by removing every connection touching them (`W`, then `X`) dragged independently again, moving nothing else.
