...
- [x] **4.1** Waiting entries on a block, multiple at once, available on Active projects.
  ...
  - **Acceptance verified 2026-09-11/12** in headless Chromium (Playwright, `npm install --no-save`) against `vite dev`, real hooks, through a hand-rolled relay translating `/rest/v1/*` into real SQL against a scratch Postgres 16 (`0001`+`0002`+`0003` applied, stub `auth.uid()`, RLS genuinely enforced via `SET LOCAL ROLE authenticated` + `request.jwt.claims`). A project with zero entries showed "Not waiting on anything" and a status change off Active succeeded (gates nothing, per 4.5). After entries were added the card read "Waiting — 2 entries"; after deleting both entries in turn (see 4.9), the block's card reverted to its plain full-Active colour with no Waiting card rendered at all, and a status change off Active succeeded again — confirming the last-entry-deleted transition, not just the first-entry-added one. No `waiting` column exists anywhere in `0002_v2.sql` or `src/types.ts`.
- [x] **4.2** Two entry types, chosen per entry: free-typed text, or one or more picked tasks. One entry can hold several picked tasks, and resolves only when every one of them is Done.
  - **Acceptance verified**: created one `kind='tasks'` entry on a project holding 2 picks (both from another project's list) and one `kind='text'` entry on the same project, confirmed by direct SQL (`waiting_entries`/`waiting_entry_tasks` row counts) and in the popup's own Waiting list, which rendered both. "Resolves only when every pick is Done" is 4.4's Ready rule, exercised there.
- [x] **4.3** The picker offers tasks from other projects **in the same Area only** — never the project's own tasks, and never another Area's.
  ...
  - **Acceptance verified**: with a task added to the project's own list first (so its exclusion is a real assertion, not an empty one), the picker's rendered text included two other same-Area projects' tasks and their project-name group headers, and did **not** include the project's own task text anywhere in the popup, nor a task from a second Area's project, nor that second Area's project name. `usePickableTasks` (`src/lib/waiting.ts`) implements this with two plain reads (blocks in the Area excluding this one, then their lists, then those lists' tasks) rather than a cross-Area query at any point.
- [x] **4.4** Ready, derived on read and never stored: the project holds at least one entry, every entry is a picked-task entry holding at least one pick, and every picked task across every entry is Done. A single free-text entry anywhere on the project means Ready can never be reached automatically — free text is cleared by hand, like any note or task.
  ...
  - **Acceptance verified, both vacuous cases built directly against the database rather than assumed:**
    - *Zero entries.* A freshly-Active project with no `waiting_entries` rows showed no "Ready"/"Not ready" label at all (not Waiting, so the question doesn't arise) and its status change off Active succeeded — `isProjectReady([])` is `false` by construction (`entries.length === 0` returns early), never reached in this state because `isProjectWaiting` is also `false`.
    - *An entry that lost its last pick to a cascade, client tidy-up never run.* Built exactly as the plan asks: picked one task from another project onto a fresh entry (1 pick, confirmed by SQL), then deleted that task **directly by SQL** — bypassing the app entirely, so `removeEmptiedWaitingEntries` never ran. The `waiting_entry_tasks` row was gone via the FK cascade but the `waiting_entries` row was still standing (1 row, 0 picks) — reopening the project's popup showed "(no tasks picked)" and no "Ready" label, and a status change off Active was refused, naming "a picked-task entry with no tasks selected". Confirms Ready does not depend on the client having tidied the row away.
  - **Acceptance verified — the whole-phase scenario (see 4.10 below) directly**: with both picked tasks Done and one free-text entry still present, the popup kept showing "Not ready" and the status-change refusal named the free-text entry by its text; deleting that free-text entry (through 4.9's confirm dialog) flipped the popup to "Ready" **immediately**, with no page reload and no status transition in between — confirmed against the database that `blocks.status` was still `'active'` at that instant, i.e. reaching Ready is a read-time computation, not a side effect of anything else firing.
- [x] **4.5** A project cannot leave Active — to either Upcoming or Done — while it is genuinely Waiting: holding at least one entry that hasn't reached the Ready state defined in 4.4. It must reach Ready first before a status change off Active is allowed.
  ...
  - **Acceptance verified**: attempting to change status away from Active while any non-Ready Waiting entry exists is refused (confirmed against the database — `blocks.status` unchanged), with an error-banner message naming what's still outstanding — tested against three distinct outstanding reasons in three separate refusals (incomplete picked tasks, a free-text entry named by its own text, and a zero-pick entry) and each message named the right one; the same project becomes changeable (`blocks.status` does update) the moment it reaches Ready. Also confirmed the reverse: a project that reaches Ready and then leaves Active (to `Done`) keeps its now-Ready entries — `4.7`'s Waiting card and colour are still shown on the Area canvas afterward, nothing clears or hides them.
- [x] **4.6** The Waiting card on the block, on the Area canvas. Folded by default, showing a one-line preview and a clear expand affordance. Expanded, it lists every entry.
  ...
  - **Acceptance verified**: folded, the card showed exactly "Waiting — 2 entries" and none of the entries' own content; clicking the fold toggle expanded it to list both entries (the tasks entry's two picks and the free-text entry's text) — confirmed by reading the card's own rendered text, not the popup's. Completing both picked tasks (through the *other* project's own popup, not this one) left both still listed on this card, each now rendering inside a `line-through` element — they do not disappear. Fixed a real bug found by this exact check: the card's `useWaitingEntries` query wasn't invalidated by a task-completion toggle happening elsewhere, so a `BlockCard` that stays mounted (unlike the popup, which remounts and refetches on every open) kept showing stale, un-struck-through picks — `useUpdateTask`/`useDeleteTask` in `src/lib/tasks.ts` now also invalidate the `['waiting-entries']` query family, since a task's completion (or deletion) can be shown on any other project's card via a pick, not just its own list.
- [x] **4.7** Waiting visual: a dimmed version of the block's normal Active colour.
  - **Acceptance verified**: a Waiting block's card class list includes `bg-blue-200` (the dimmed counterpart to Active's `bg-blue-500`) and never `bg-blue-500` or the Done/Upcoming `bg-gray-300` while it holds any entry — checked via the rendered `class` attribute rather than computed colour, since Tailwind v4 computes backgrounds as `oklch(...)`, not `rgb(...)`. Also checked that the treatment survives a status change off Active once Ready (per 4.5's note that leaving Active doesn't clear entries), and that it reverts to the block's own plain status colour the moment the last entry is deleted (4.1).
- [x] **4.8** Ready visual: a distinct badge or dot layered on top of the Waiting treatment — deliberately not another outline, because it has to be noticed rather than blend into the colour state.
  - **Acceptance verified**: a small green circular checkmark badge (`aria-label="Ready"`), absolutely positioned over the card's corner, was absent while a free-text entry made the project not-Ready and present the moment it became Ready — screenshotted both states. It is additive to 4.7's colour treatment (the card is still `bg-blue-200` underneath it), not a second outline or border.
- [x] **4.9** Entry delete goes through the confirm dialog.
  ...
  - **Acceptance verified**: clicking Delete on an entry opened the same confirm dialog every other delete in the app uses; Cancel left the entry in the database (confirmed by SQL) and the popup's list; Confirm removed it. Doing the same to a task's *own* pick (via `ListCard`, 3.5) still raises only 3.5's single dialog, unchanged this phase.
- [x] **4.10** Waiting is fully independent of Connect: picking a task never creates a connection, and no connection ever creates an entry.
  - Verified by construction — `src/lib/waiting.ts` and `WaitingEntryCreatePopup.tsx` never read or write `connections`, and Connect does not exist yet (Phase 6), so there is nothing on the other side to couple to.
  ...
  - **Verified exactly as written, in one continuous run**: Project B held a `kind='text'` entry ("Waiting on vendor quote") and a `kind='tasks'` entry picking two tasks from another project — both shown, expanded, in the popup and (separately) on the Area-canvas card. Completing both picked tasks (elsewhere, in their own project's popup) struck them through on both views but the popup kept reading "Not ready" and a status-change attempt was refused, naming the free-text entry specifically — Ready was not reached. Deleting the free-text entry (through 4.9's confirm dialog) immediately flipped the popup to "Ready", with `blocks.status` unchanged at that instant (checked by direct query) — no transition fired or was needed.

**Whole-phase verification, 2026-09-11/12.** Built on top of Phase 3's own harness pattern (scratch Postgres 16, `0001`+`0002`+`0003` applied, a hand-rolled relay translating `/rest/v1/*` into real parameterized SQL under genuinely enforced RLS, a seeded `localStorage` session, headless Chromium via Playwright installed `--no-save`) — extended this phase to also translate PostgREST's embedded-resource `select` syntax (`waiting_entries` embedding `blocks(name)`, `waiting_entry_tasks` embedding `tasks(text, completed)`), since Phase 4 is the first to need it from the client. **52/52 checks passed**, covering every acceptance bullet above plus a same-Area/cross-Area picker check, both 4.4 vacuous cases built directly against the database, three distinct 4.5 refusal messages, and an RLS sanity probe on the two tables Phase 4 makes live for the first time (a non-owner claim reads zero rows and its insert is rejected with `42501` — no schema changed this phase, so this confirms Phase 2a's policies rather than re-proving them). Zero browser console or page errors across the whole run. Two screenshots taken (the Area canvas mid-scenario, showing a struck-through Waiting card with its Ready badge next to an untouched Active block and a folded Waiting card; the project popup with its Waiting section and an empty list). One real app bug was found and fixed during this verification, not just a test artifact — recorded under 4.6 above.
  - **Not checked:** the live Supabase project (this container had no `.env` at session start, by design, same as every phase since 2a — a local-only `.env` pointing at the real project host with a placeholder anon key was written for the `vite dev` run and deleted before finishing, never committed). `npm run build` (477.8 kB / 134.4 kB gzip, `.env` present — this is the real-client-path figure Phase 3's own review recorded, not the "no `.env`" figure most phases quote, which tree-shakes the entire Supabase client away as statically-unreachable dead code behind the `if (!url || !key) throw` guard in `src/lib/supabase.ts`; confirmed by toggling `.env`'s presence and rebuilding, both matching exactly) and `npm run lint` both pass clean. No new dependency in `package.json`/`package-lock.json` — `playwright`/`pg` were installed `--no-save` for the harness and never committed; no `.env`, harness script, or scratch SQL was committed. Phase 5 (Focused), 6 (Connect) and 7 (Filter/content marker) — out of scope this phase, per the spec.
