...
- [x] **0.3** Supabase client wired to the dedicated project; URL and anon key from `.env` (`.env` gitignored, `.env.example` committed).
  ...
  - **Acceptance verified 2026-09-09** against the live project (ref `ieszecmiijxkcrxirwuk`) via `npm run verify:rls`: all four tables returned no rows with no session, all four writes were refused with PostgREST `42501` (row-level security policy), and signup was refused with `422 signup_disabled`. The policies are pinned to the owner's UID rather than merely to "signed in," because Supabase allows email signups by default and the anon key ships publicly in the client bundle; disabling signups in the dashboard is the other half and was confirmed there directly.
